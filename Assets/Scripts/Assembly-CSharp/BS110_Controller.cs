...
public class BS110_Controller : EnemyControllerBase, IManagedUpdateBehavior
{
	...
	[SerializeField]
	private string MistFXName = "fxduring_MHR2_mist_000";

	private bool hasPlayMistFx;
	...
	protected override void Awake()
	{
		...
		BuffCollide = OrangeBattleUtility.FindChildRecursive(ref target, BuffCollideName, true).gameObject.AddOrGetComponent<CollideBulletHitSelf>();
	...
	public override void SetActive(bool isActive)
	{
		...
		if (isActive)
		{
			...
			BuffCollide.UpdateBulletData(EnemyWeapons[4].BulletData, "", base.gameObject.GetInstanceID());
	...
	public override void SetPositionAndRotation(Vector3 pos, bool bBack)
	{
		...
		{
			leftTop = new Vector3(position.x - raycastHit2D.distance + 0.5f, position.y + raycastHit2D3.distance - 0.25f, 0f);
			rightTop = new Vector3(position.x + raycastHit2D2.distance - 0.5f, position.y + raycastHit2D3.distance - 0.25f, 0f);
			CenterPos = new Vector3((leftTop.x + rightTop.x) / 2f, (raycastHit2D3.point.y + raycastHit2D4.point.y) / 2f, 0f);
		}
	}
	...
	public override ObscuredInt Hurt(HurtPassParam tHurtPassParam)
	{
		...
		if (!hasPlayMistFx && (int)obscuredInt <= (int)MaxHp / 2 && (int)Hp >= 0)
		{
			PlayBossSE("BossSE", "bs008_kame09");
			hasPlayMistFx = true;
