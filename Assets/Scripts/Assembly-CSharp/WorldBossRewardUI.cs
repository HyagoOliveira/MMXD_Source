...
public class WorldBossRewardUI : OrangeUIBase
{
	...
	public void Setup(int nEventID, int nScore, int nStep, STAGE_TABLE tStage, int nRank)
	{
	...
	public void OnScoreRewardPage(bool bIsOn)
	{
		...
		ScoreNumRootText.text = MonoBehaviourSingleton<LocalizationManager>.Instance.GetStr("RAID_CONTRIBUTION");
		ScoreNumRootNum.text = nNowScore.ToString();
		HintMsgText.text = "";
		...
		KeyValuePair<int, MISSION_TABLE>[] array = (from q in ManagedSingleton<OrangeDataManager>.Instance.MISSION_TABLE_DICT
			where q.Value.n_TYPE == 102 && q.Value.n_SUB_TYPE == tET.n_POINT && q.Value.n_CONDITION == 121 && q.Value.n_CONDITION_X == tET.n_ID
			orderby q.Value.n_CONDITION_Y
			select q).ToArray();
		for (int i = 0; i < array.Length; i++)
		{
			bool bCompleted = ManagedSingleton<MissionHelper>.Instance.CheckMissionCompleted(array[i].Value.n_ID);
			bool bRetrieved = ManagedSingleton<MissionHelper>.Instance.CheckMissionRewardRetrieved(array[i].Value.n_ID);
	...
	public void OnRankPage(bool bIsOn)
	{
		...
		InitLineStatus();
		BtnGetAll.gameObject.SetActive(false);
		IOrderedEnumerable<KeyValuePair<int, MISSION_TABLE>> source = from q in ManagedSingleton<OrangeDataManager>.Instance.MISSION_TABLE_DICT
			where q.Value.n_TYPE == 110 && q.Value.n_SUB_TYPE == tET.n_RANKING
			orderby q.Value.n_CONDITION_Y
			select q;
		ScoreNumRoot.SetActive(true);
		ScoreNumRootText.text = MonoBehaviourSingleton<LocalizationManager>.Instance.GetStr("EVENT_STAGE_RANKING");
		...
		KeyValuePair<int, MISSION_TABLE>[] array = source.ToArray();
	...
	public void OnChallengePage(bool bIsOn)
	{
		if (bIsOn)
		{
			...
			ScoreNumRoot.SetActive(false);
			HintMsgText.text = "";
			List<GACHA_TABLE> listGachaByGroup = ManagedSingleton<ExtendDataHelper>.Instance.GetListGachaByGroup(tNowStage.n_GET_REWARD);
	...
	public void OnSuccessPage(bool bIsOn)
	{
		if (!bIsOn)
		{
			return;
		...
		ScoreNumRoot.SetActive(true);
		ScoreNumRootText.text = MonoBehaviourSingleton<LocalizationManager>.Instance.GetStr("RAID_CRUSADE_STEP");
		ScoreNumRootNum.text = nNowStep.ToString();
		HintMsgText.text = MonoBehaviourSingleton<LocalizationManager>.Instance.GetStr("RAID_CRUSADE_TIP");
		KeyValuePair<int, MISSION_TABLE>[] array = (from q in ManagedSingleton<OrangeDataManager>.Instance.MISSION_TABLE_DICT
			where q.Value.n_TYPE == 109 && q.Value.n_CONDITION_X == tET.n_ID && q.Value.n_SUB_TYPE == tET.n_RANKING
			orderby q.Value.n_CONDITION_Y
			select q).ToArray();
		int num = 0;
		int ranknum = 0;
		for (int i = 0; i < array.Length; i++)
		{
			if (num < array[i].Value.n_CONDITION_Y)
			{
				num = array[i].Value.n_CONDITION_Y;
				ranknum = ((num >= 4) ? 4 : num);
				AddBattleRank(ranknum, num);
			}
			if (array[i].Value.n_ITEMID_1 != 0)
			{
				AddRankItem(array[i].Value.n_ITEMID_1, array[i].Value.n_ITEMCOUNT_1, i, ranknum);
			}
			if (array[i].Value.n_ITEMID_2 != 0)
			{
				AddRankItem(array[i].Value.n_ITEMID_2, array[i].Value.n_ITEMCOUNT_2, i, ranknum);
			}
			if (array[i].Value.n_ITEMID_3 != 0)
			{
				AddRankItem(array[i].Value.n_ITEMID_3, array[i].Value.n_ITEMCOUNT_3, i, ranknum);
			}
		}
	}
	...
	private void AddItemWithGetBtn(int itemId, int amount, int score, int n_ID, bool bCompleted, bool bRetrieved)
	{
		...
		transform.transform.Find("TextScore").GetComponent<OrangeText>().text = string.Format(MonoBehaviourSingleton<LocalizationManager>.Instance.GetStr("RAID_CONTRIBUTION_POINT"), score);
