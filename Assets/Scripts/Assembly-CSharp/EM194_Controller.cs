...
public class EM194_Controller : EnemyControllerBase, IManagedUpdateBehavior
{
	...
	private bool SniperMode = true;

	[SerializeField]
	private float DisToCloseAtk = 5f;
	...
	private void SetStatus(MainStatus mainStatus, SubStatus subStatus = SubStatus.Phase0)
	{
		...
		if (SniperMode)
		{
			SetStatusForLongRangeAtk();
		}
		else
		{
			SetStatusForCloseAtk();
		}
		AiTimer.TimerStart();
		UpdateAnimation();
	}
	...
	private void SetStatusForCloseAtk()
	{
		switch (_mainStatus)
		{
		case MainStatus.Idle:
		...
		case MainStatus.Fall:
			...
			case SubStatus.Phase0:
				...
				FallFrame = GameLogicUpdateManager.GameFrame + (int)(FallTimer * 20f);
				break;
			case SubStatus.Phase1:
				SetWallSide(2);
				break;
		...
		case MainStatus.Walk:
			...
			case WallSide.Down:
				...
				break;
			case WallSide.Left:
				_velocity.y = WalkSpeed * base.direction;
				break;
			case WallSide.Right:
				_velocity.y = WalkSpeed * base.direction;
				break;
	...
	private void UpdateForCloseAtk()
	{
		switch (_mainStatus)
		{
		case MainStatus.Idle:
		...
		case MainStatus.Fall:
			...
			case SubStatus.Phase1:
				if (Controller.Collisions.below)
				{
					...
					SetStatus(MainStatus.Fall, SubStatus.Phase2);
				}
				break;
			case SubStatus.Phase2:
				if (_currentFrame > 1f)
		...
		case MainStatus.Walk:
			...
			case WallSide.Up:
				...
				if ((bool)Physics2D.Raycast(_transform.position, Vector2.down, 5f, targetMask))
				{
			...
			case WallSide.Right:
				if (isAtking)
				{
					...
					_velocity.y = RunSpeed * base.direction;
					break;
				}
	...
	private void SetStatusForLongRangeAtk()
	{
		switch (_mainStatus)
		{
		case MainStatus.Idle:
	...
	private void UpdateForLongRangeAtk()
	{
		switch (_mainStatus)
		{
		case MainStatus.Idle:
			...
			break;
		case MainStatus.Atk:
			if ((bool)Target && Target._transform.position.y < _transform.position.y && Vector2.Distance(Target.Controller.GetRealCenterPos(), ShootPoint.position) <= DisToCloseAtk)
			{
				SniperMode = false;
				SetStatus(MainStatus.Fall);
			}
	...
	private bool CheckHitPlayer()
	{
		...
		if ((bool)Physics2D.OverlapBox(point, size, 0f, LayerMask.GetMask("Player")))
		{
			return true;
		}
		return false;
