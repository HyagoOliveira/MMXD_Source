...
public class BS089_Controller : EnemyControllerBase, IManagedUpdateBehavior
{
	...
	private CollideBullet _bodycollide;
	...
	protected override void Start()
	{
		...
		_bodycollide = OrangeBattleUtility.FindChildRecursive(ref target, "BodyCollide", true).gameObject.AddOrGetComponent<CollideBullet>();
	...
	{
		if (EnemyWeapons != null && !_bodycollide.IsActivate)
		{
			_bodycollide.UpdateBulletData(EnemyWeapons[0].BulletData);
			_bodycollide.SetBulletAtk(null, selfBuffManager.sBuffStatus, EnemyData);
			_bodycollide.Active(targetMask);
		}
		switch (_mainStatus)
		{
		case MainStatus.Dead:
			...
			if (_bodycollide.IsActivate)
			{
				_bodycollide.BackToPool();
			}
			break;
		case MainStatus.Idle:
		case MainStatus.Hurt:
