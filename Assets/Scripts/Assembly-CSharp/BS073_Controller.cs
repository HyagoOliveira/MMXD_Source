...
public class BS073_Controller : EnemyControllerBase, IManagedUpdateBehavior
{
	...
	[SerializeField]
	private float SK5AssaultTime = 1.6f;
	...
	private void SetStatus(MainStatus mainStatus, SubStatus subStatus = SubStatus.Phase0)
	{
		...
		case MainStatus.Debut:
			...
			case SubStatus.Phase0:
				...
				_efx_SpotLight.Stop();
				EndFrame = GameLogicUpdateManager.GameFrame + 40;
				moveperframe = 0.1f;
				_projectorTransform.position = new Vector3(_projectorTransform.position.x, ModelTransform.position.y + _projectorHeight, _projectorTransform.position.z);
				break;
		...
		case MainStatus.Skill0:
			if (_subStatus == SubStatus.Phase0)
			{
				...
				Vector3.Angle(Vector3.right * base.direction, vector5);
				lastdistance = Vector3.Distance(_transform.position, EndPos);
				_velocity = new VInt3(vector5 * 0.001f * MoveSpeed);
			}
			break;
		...
		case MainStatus.Skill3:
			if (_subStatus == SubStatus.Phase0)
			{
				...
				_collideBullet.SetBulletAtk(null, selfBuffManager.sBuffStatus, EnemyData);
				Vector3 vector4 = Quaternion.Euler(0f, 0f, (float)(-base.direction) * TurnToward) * Vector3.up;
				if (TurnToward > 180f)
				{
				...
				_velocity = new VInt3(vector4 * 0.001f * AssaultSpeed);
		...
		case MainStatus.Skill4:
			if (_subStatus == SubStatus.Phase0)
			{
				...
				float value2 = Vector3.Angle(Vector3.up, vector3);
	...
	public override void LogicUpdate()
	{
		...
		case MainStatus.Debut:
			...
			case SubStatus.Phase0:
				...
				{
					_projectorTransform.position += Vector3.down * moveperframe;
				}
				else
				{
			...
			case SubStatus.Phase3:
				if (GameLogicUpdateManager.GameFrame < EndFrame)
				{
					_projectorTransform.position += Vector3.up * moveperframe;
				}
				else
				{
		...
		case MainStatus.Skill0:
		{
			...
			if (num2 > lastdistance)
			{
				...
				if ((bool)Target)
				{
					num2 = Vector3.Distance(_transform.position, Target._transform.position);
					if (num2 < JudgeDis)
					{
	...
	{
		float x = base.transform.position.x + 10f;
		float x2 = base.transform.position.x - 10f;
		float y = base.transform.position.y + 10f;
		float y2 = base.transform.position.y - 10f;
