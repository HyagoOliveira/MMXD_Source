...
public class BS070_Controller : EnemyControllerBase, IManagedUpdateBehavior
{
	...
	private int event997;

	private int event998;

	private int event999;

	private int nDeadWaitFramCount;
	...
	public override void LogicUpdate()
	{
		...
		case MainStatus.Debut:
			...
			case SubStatus.Phase3:
				...
				{
					cancallevent = true;
					event997 = GameLogicUpdateManager.GameFrame + 200;
					event998 = GameLogicUpdateManager.GameFrame + 400;
					event999 = GameLogicUpdateManager.GameFrame + 600;
		...
		if (cancallevent)
		{
			if (GameLogicUpdateManager.GameFrame > event997)
			{
				CallEventEnemyPoint(997);
				event997 += 600;
			}
			else if (GameLogicUpdateManager.GameFrame > event998)
			{
				CallEventEnemyPoint(998);
				event998 += 600;
			}
			else if (GameLogicUpdateManager.GameFrame > event999)
			{
	...
	public override void Unlock()
	{
		...
		if (InGame && ((int)Hp > 0 || (_mainStatus == MainStatus.Dead && _subStatus == SubStatus.Phase0)))
		{
			Activate = true;
		}
	}
