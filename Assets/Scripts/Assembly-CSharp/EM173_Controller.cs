...
public class EM173_Controller : EnemyControllerBase, IManagedUpdateBehavior
{
	...
	[SerializeField]
	[Tooltip("手舉高的速度")]
	private int HandOnSpeed = 12000;

	[SerializeField]
	[Tooltip("手舉高每幀減少的速度")]
	private int HandOnReduce = 1200;

	private CollideBullet ThunderCollider;

	private bool HasThunder;
	...
	private bool CanUseThunder = true;

	[SerializeField]
	private SkinnedMeshRenderer HandPart;
	...
	protected override void Awake()
	{
		...
		if (HandPart == null)
		{
			HandPart = OrangeBattleUtility.FindChildRecursive(ref target, "HandMesh_1").GetComponent<SkinnedMeshRenderer>();
		}
	...
	{
		MonoBehaviourSingleton<FxManager>.Instance.PreloadFx("fxuse_MHR_Magnamalo_above_hand_000", 2);
		MonoBehaviourSingleton<FxManager>.Instance.PreloadFx("fxuse_MHR_Magnamalo_under_hand_000", 2);
		MonoBehaviourSingleton<FxManager>.Instance.PreloadFx("fxhit_MHR_Magnamalo_hitground_000", 2);
	}
	...
	public override void UpdateStatus(int nSet, string smsg, Callback tCB = null)
	{
		...
		if (smsg != null && smsg != "")
		{
			...
			MoveSpeed = (int)((float)(netSyncData.nParam0 * 500) * SpeedOverride);
	...
	private void SetStatus(MainStatus mainStatus, SubStatus subStatus = SubStatus.Phase0)
	{
		...
		case MainStatus.Swing:
			...
			case SubStatus.Phase0:
				_velocity.y = HandOnSpeed;
				_velocity.x = 0;
				SwingStart = Controller.LogicPosition.vec3;
				base.SoundSource.PlaySE("BossSE04", "bs035_onkosig12");
				break;
			case SubStatus.Phase1:
				SwingEnd.x = TargetPos.vec3.x;
				SkillTimer.TimerStart();
				SwingTarget.position = new Vector3(TargetPos.vec3.x, SwingEnd.y, 0f);
				SwingTarget.localRotation = Quaternion.Euler(90f, 90f, 0f);
				PredictSpriteRenderer.enabled = true;
				break;
			case SubStatus.Phase2:
				_velocity.x = (int)((float)(TargetPos.x - Controller.LogicPosition.x) * 4f);
				_velocity.y = -15000;
				IgnoreGravity = false;
			...
			{
				base.SoundSource.PlaySE("BossSE04", "bs035_onkosig11");
				IgnoreGravity = true;
				SkillTimer.TimerStart();
		...
		case MainStatus.Thunder:
			...
			case SubStatus.Phase0:
				_velocity.x = 0;
				ActionFrame = GameLogicUpdateManager.GameFrame + 6;
				break;
			case SubStatus.Phase1:
				ActionFrame = GameLogicUpdateManager.GameFrame + 20;
				if (CanUseThunder)
				{
					MonoBehaviourSingleton<FxManager>.Instance.Play<FxBase>("fxuse_MHR_Magnamalo_above_hand_000", _transform.position + new Vector3(0f, 0f, 0f), Quaternion.Euler(0f, 0f, 0f), new object[1]
					{
						new Vector3(1f, 1f, 1f)
					});
					MonoBehaviourSingleton<FxManager>.Instance.Play<FxBase>("fxuse_MHR_Magnamalo_under_hand_000", _transform.position + new Vector3(0f, -1f, 0f), Quaternion.Euler(0f, 0f, 0f), new object[1]
					{
						new Vector3(1f, 1f, 1f)
					});
				}
				break;
			case SubStatus.Phase2:
				ActionFrame = GameLogicUpdateManager.GameFrame + 12;
				break;
			}
			break;
		case MainStatus.Die:
			_velocity.x = 0;
			_velocity.y = 0;
			base.SoundSource.PlaySE("BossSE04", "bs035_onkosig02_stop");
			break;
		}
		AiTimer.TimerStart();
	}
	...
	public override void LogicUpdate()
	{
		...
		case MainStatus.Swing:
			...
			case SubStatus.Phase2:
				...
				if (_transform.position.y < SwingEnd.y + 2f)
				{
					...
					MonoBehaviourSingleton<FxManager>.Instance.Play<FxBase>("fxhit_MHR_Magnamalo_hitground_000", vector, Quaternion.Euler(0f, 0f, 0f), new object[1]
					{
						new Vector3(1f, 1f, 1f)
					});
				}
			...
			case SubStatus.Phase3:
				...
				if (SwingCollide.IsActivate && SkillTimer.GetMillisecond() > 100)
				{
		...
		case MainStatus.Thunder:
			...
			case SubStatus.Phase0:
				...
				break;
			case SubStatus.Phase1:
				if (GameLogicUpdateManager.GameFrame > ActionFrame + 28)
				{
					if (HasThunder)
					{
						HasThunder = false;
				...
				else if (GameLogicUpdateManager.GameFrame > ActionFrame && !ThunderCollider.IsActivate && CanUseThunder)
				{
					HasThunder = true;
					base.SoundSource.PlaySE("BossSE04", "bs035_onkosig02_lp");
	...
	public override void SetActive(bool isActive)
	{
		...
		if (isActive)
		{
			...
			if (base.direction == 1)
			{
				RightRange = NextXPos + 2f * (float)base.direction;
				LeftRange = NextXPos + (Distance + 1.2f) * (float)(-base.direction);
			}
			else
			{
				LeftRange = NextXPos + 2f * (float)base.direction;
	...
	public override void SetPositionAndRotation(Vector3 pos, bool bBack)
	{
		...
		ModelTransform.localScale = new Vector3(Math.Abs(ModelTransform.localScale.x) * (float)(-base.direction), ModelTransform.localScale.y, 0f - Math.Abs(ModelTransform.localScale.z));
	...
	public void SetWeak(float speed)
	{
		SpeedOverride = speed;
		CanUseThunder = false;
		HandPart.enabled = false;
		if (HasThunder)
		{
			ThunderCollider.BackToPool();
		}
	...
	public Transform GetHandPart()
	{
		return HandPart.transform;
	}
}
