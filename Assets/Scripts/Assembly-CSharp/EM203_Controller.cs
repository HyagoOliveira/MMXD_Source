...
public class EM203_Controller : EnemyControllerBase, IManagedUpdateBehavior
{
	...
	[SerializeField]
	private int Skill0ShootTimes1 = 1;

	[SerializeField]
	private int Skill0ShootTimes2 = 2;

	[SerializeField]
	private int Skill0ShootTimes3 = 3;
	...
	public override void LogicUpdate()
	{
		...
		case MainStatus.Debut:
			...
			case SubStatus.Phase1:
				...
				if (MantleMesh != null && MantleMesh.enabled && _currentFrame > 0.5f)
				{
					SwitchMesh(MantleMesh, false);
				}
				if (_currentFrame > 1f)
		...
		case MainStatus.Skill0:
			...
			case SubStatus.Phase3:
				if (!HasActed && _currentFrame > ActionAnimatorFrame)
				{
					...
					case Element.Normal:
					{
						EndPos = CenterPos + Quaternion.Euler(0f, 0f, BlackPosAngle) * (Vector3.up * 3f);
		...
		case MainStatus.Skill2:
			...
			case SubStatus.Phase2:
				...
				{
					HasActed = true;
					_velocity = new VInt3((EndPos - NowPos).normalized) * Skill2DashSpeed * 0.001f;
				}
				if (_currentFrame > 1f)
				{
		...
		case MainStatus.Skill3:
			...
			case SubStatus.Phase1:
				...
				break;
			case SubStatus.Phase2:
				if (BlackList.Count < 1 && YellowList.Count < 1)
				{
					SetDie();
				}
				break;
	...
	public void SetRoomSize(Vector3 maxpos, Vector3 minpos)
	{
		MaxPos = maxpos;
		MinPos = minpos;
	...
	private EnemyControllerBase SpawnEnemy(Vector3 SpawnPos, int weaponid)
	{
		...
		if ((bool)enemyControllerBase)
		{
			...
			{
				eM158_Controller.SetNextPos(SpawnPos + Quaternion.Euler(0f, 0f, EM158PosAngle) * (Vector3.up * 1.5f));
	...
	private void CheckBlackList()
	{
		...
		for (int i = 0; i < BlackList.Count; i++)
		{
			...
			else if (GameLogicUpdateManager.GameFrame > BlackList[i].Item2.CDFrame)
			{
				...
				if (BlackList[i].Item2.ShootTime < 1)
				{
					BlackList[i].Item1.SetBackToPool();
					BlackList.Remove(BlackList[i]);
				}
	...
	private void CheckYellowList()
	{
		...
		for (int i = 0; i < YellowList.Count; i++)
		{
			...
			else if ((bool)Target && GameLogicUpdateManager.GameFrame > YellowList[i].Item2.CDFrame)
			{
				...
				if (YellowList[i].Item2.ShootTime < 1)
				{
					YellowList[i].Item1.SetBackToPool();
					YellowList.Remove(YellowList[i]);
				}
