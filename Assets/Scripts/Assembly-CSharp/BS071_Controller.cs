...
public class BS071_Controller : EnemyControllerBase, IManagedUpdateBehavior
{
	...
	private float SummonTime = 25f;

	private float[] SummonDelay = new float[5] { 0f, 1f, 2f, 3f, 6f };

	private int[] SummonFrame = new int[5];
	...
	public override void LogicUpdate()
	{
		...
		case MainStatus.Debut:
			...
			case SubStatus.Phase4:
				if (_unlockReady)
				{
					...
					for (int i = 0; i < 5; i++)
					{
						SummonFrame[i] = GameLogicUpdateManager.GameFrame + (int)((SummonTime + SummonDelay[i]) * 20f);
					}
		...
		for (int j = 0; j < 5; j++)
		{
			if (CanSummon && GameLogicUpdateManager.GameFrame >= SummonFrame[j])
			{
				CallEventEnemyPoint(997 - j);
				SummonFrame[j] = GameLogicUpdateManager.GameFrame + (int)(SummonTime * 20f);
			}
		}
	}
