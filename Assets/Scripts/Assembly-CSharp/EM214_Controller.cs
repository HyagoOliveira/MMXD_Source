...
public class EM214_Controller : EnemyControllerBase, IManagedUpdateBehavior
{
	...
	[SerializeField]
	private ParticleSystem SkillFx1;

	[SerializeField]
	private ParticleSystem SkillFx2;
	...
	private void SetStatus(MainStatus mainStatus, SubStatus subStatus = SubStatus.Phase0)
	{
		...
		case MainStatus.Skill:
			...
			case SubStatus.Phase0:
				_velocity = VInt3.zero;
				ActionFrame = GameLogicUpdateManager.GameFrame + (int)(SkillActTime1 * 20f);
				break;
			case SubStatus.Phase1:
				_collideBullet.UpdateBulletData(EnemyWeapons[1].BulletData);
				_collideBullet.SetBulletAtk(null, selfBuffManager.sBuffStatus, EnemyData);
				_collideBullet.Active(targetMask);
				PlayBossSE("BossSE", "bs011_panda11_lp");
				SkillFx1.Play();
				_velocity = VInt3.zero;
				ActionFrame = GameLogicUpdateManager.GameFrame + (int)(SkillActTime2 * 20f);
				break;
			}
			break;
		}
	...
	public override void LogicUpdate()
	{
		...
		case MainStatus.Skill:
			...
			case SubStatus.Phase0:
			{
				...
				if (Vector2.Distance(Target.GetTargetPoint(), NowPos) < 0.3f)
				{
			...
			case SubStatus.Phase1:
				if (GameLogicUpdateManager.GameFrame >= ActionFrame)
				{
					...
					SkillFx2.Play();
					GoReturn = true;
					PlaySE("BossSE", "bs011_panda11_stop");
					SetStatus(MainStatus.Move);
				}
				break;
			}
