...
public class BS091_Controller : EnemyControllerBase, IManagedUpdateBehavior
{
	...
	private List<ValueTuple<int, int>> SummonList = new List<ValueTuple<int, int>>();

	[SerializeField]
	private float Event995 = 3f;

	[SerializeField]
	private float Event996 = 3f;

	[SerializeField]
	private float Event997 = 3f;

	[SerializeField]
	private float Event998 = 3f;

	[SerializeField]
	private float Event999 = 3f;
	...
	private void HashAnimation()
	{
		...
		_animationHash[4] = Animator.StringToHash("BS047@shield_on_back_dead_loop");
	...
	private void UpdateRandomState()
	{
		...
		case MainStatus.NextAction:
			...
			if (flag)
			{
				...
				if ((uint)aiState <= 1u)
				{
					mainStatus = (MainStatus)((_lateAction != MainStatus.Skill_01 && _lateAction != MainStatus.Skill_04) ? ((!_takeShield) ? OrangeBattleUtility.Random(3, 6) : OrangeBattleUtility.Random(3, 7)) : OrangeBattleUtility.Random(4, 6));
				}
			}
			break;
	...
	public override void LogicUpdate()
	{
		...
		case MainStatus.Skill_01:
			...
			case SubStatus.Phase2:
				if (_currentFrame > 1f)
				{
					...
					PlayBossSE("BossSE02", "bs012_vajur00");
				}
				break;
			case SubStatus.Phase3:
		...
		case MainStatus.Skill_02:
			...
			case SubStatus.Phase0:
				...
				break;
			case SubStatus.Phase1:
				if (!_shootBullet && _currentFrame > 0.35f)
				{
					_shootBullet = true;
		...
		{
			ValueTuple<int, int> valueTuple = SummonList[i];
			if (!_canSummon)
			{
				i--;
				SummonList.RemoveAt(0);
			}
			else if (_canSummon && GameLogicUpdateManager.GameFrame > valueTuple.Item1)
			{
				MonoBehaviourSingleton<OrangeBattleUtility>.Instance.CallSummonEnemyEvent(_transform, valueTuple.Item2);
				i--;
				SummonList.RemoveAt(0);
			}
		}
	...
	private void SetStatus(MainStatus mainStatus, SubStatus subStatus = SubStatus.Phase0)
	{
		...
		case MainStatus.Skill_01:
			...
			case SubStatus.Phase1:
				...
				if ((bool)Target)
				{
					int num2 = Mathf.RoundToInt((float)(Target.Controller.LogicPosition.y - Controller.LogicPosition.y + 1000) * 100f / (float)Mathf.Abs(_maxGravity.i));
					if (num2 > _jupmpSpeed.y)
					{
						_velocity.y = num2 * 1000;
					}
					else
					{
			...
			case SubStatus.Phase2:
				...
				base.SoundSource.ForcePlaySE("BossSE02", "bs012_vajur02");
			...
			case SubStatus.Phase3:
				...
				if (base.direction == 1)
				{
					num = GameLogicUpdateManager.GameFrame + (int)(20f * Event996);
					SummonList.Add(new ValueTuple<int, int>(num, 996));
				}
				else
				{
					num = GameLogicUpdateManager.GameFrame + (int)(20f * Event998);
					SummonList.Add(new ValueTuple<int, int>(num, 998));
				}
				break;
			case SubStatus.Phase4:
		...
		case MainStatus.Skill_04:
			...
			case SubStatus.Phase1:
				...
				if (base.direction == 1)
				{
					num = GameLogicUpdateManager.GameFrame + (int)(20f * Event995);
					SummonList.Add(new ValueTuple<int, int>(num, 995));
				}
				else
				{
					num = GameLogicUpdateManager.GameFrame + (int)(20f * Event997);
					SummonList.Add(new ValueTuple<int, int>(num, 997));
				}
		...
		case MainStatus.ShieldOn:
			_takeShield = true;
			_shieldTransform.gameObject.SetActive(true);
			num = GameLogicUpdateManager.GameFrame + (int)(20f * Event999);
			PlaySE("BossSE02", "bs012_vajur12");
			SummonList.Add(new ValueTuple<int, int>(num, 999));
			break;
		case MainStatus.ShieldOff:
			_takeShield = false;
			_shieldTransform.gameObject.SetActive(false);
