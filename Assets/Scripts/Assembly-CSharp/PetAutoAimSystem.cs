...
public class PetAutoAimSystem : MonoBehaviour
{
	...
	private bool _IgnoreInsideScreen;

	public bool Enabled;

	protected bool _useManualTarget = true;
	...
	public void SetTarget(Collider2D col, bool isPriority = false)
	{
		if (col == null)
		{
	...
	public void SetTarget(IAimTarget target, bool isPriority = false)
	{
		if (target == null)
		{
			PriorityTarget = false;
	...
	public bool CheckTargetInList(IAimTarget target)
	{
	...
	{
		if (!col.isTrigger && 1 << col.gameObject.layer == ((1 << col.gameObject.layer) & (int)targetMask))
		{
			IAimTarget componentInParent = col.gameObject.GetComponentInParent<IAimTarget>();
			if (componentInParent != null && !_targetList.Contains(componentInParent))
			{
	...
	public IAimTarget GetClosestTarget()
	{
		...
		{
			if ((bool)target.AimTransform && target.AllowAutoAim)
			{
	...
	public void SetTargetByNetSerialID(string netSerialId)
	{
		foreach (IAimTarget target in _targetList)
		{
			if ((bool)target.AimTransform)
			{
				StageObjBase stageObjBase = target as StageObjBase;
				if (stageObjBase != null && stageObjBase.sNetSerialID == netSerialId)
				{
					SetTarget(target);
					break;
				}
	...
	private bool IsInsideScreen(Vector3 targetWorldPosition)
	{
		if (_IgnoreInsideScreen)
		{
			return true;
		}
	...
	public void SetIgnoreInsideScreen(bool enable)
	{
		_IgnoreInsideScreen = enable;
	}
	...
	public void SetUseManualTarget(bool enable)
	{
		_useManualTarget = enable;
	}

	public void UpdatePriorityTarget()
	{
		if (PriorityTarget && (AutoAimTarget == null || !IsInsideScreen(GetTargetPoint())))
		{
			PriorityTarget = false;
			AutoAimTarget = null;
		}
