...
public class CardDeployGoCheck : MonoBehaviour
{
	[SerializeField]
	...
	public bool bNeedUpdateGoCheckPower;
	...
	public void Setup(CharacterInfo info, bool bDeploy = false, bool bSetDeploy = false)
	{
		...
		if (bIsSetDeploy)
		{
			...
			CardDeployMeunRoot.Setup(true, true);
		}
		else if (bIsDeploy)
		{
			TestDeployInit();
			CardDeployMeunRoot.gameObject.SetActive(true);
			CardDeployMeunRoot.Setup(false, true);
			if (bNeedUpdateGoCheckPower)
			{
				bNeedUpdateGoCheckPower = false;
				GoCheckUI uI = MonoBehaviourSingleton<UIManager>.Instance.GetUI<GoCheckUI>("UI_GoCheck");
				if (uI != null)
				{
					uI.CardDeployScriptRoot.OnCardInfoUIClose();
					uI.SetMainSubWeapon();
				}
			}
		}
	...
	public void OnChangeDeployName()
	{
		MonoBehaviourSingleton<UIManager>.Instance.LoadUI("UI_InputText", delegate(InputTextUI ui)
		{
			...
			ui.Setup(ChangeDeployName, "--change name--");
		});
	}
	...
	{
		NetCharacterInfo netInfo = CuttentCharaterInfo.netInfo;
		int[] array = new int[3];
		...
		if (netInfo.State == 1)
		{
			ManagedSingleton<PlayerNetManager>.Instance.EquipCardDeployReq(CuttentCharaterInfo.netInfo.CharacterID, array.ToList(), delegate
			{
				Setup(CuttentCharaterInfo, bIsDeploy, bIsSetDeploy);
			});
		}
	...
	public void OnClickDeployBtn(int idx)
	{
		...
		if (bIsDeploy && !bIsSetDeploy)
		{
			...
			{
				string text = ManagedSingleton<PlayerNetManager>.Instance.dicCardDeployNameInfo[CurrentDeployIndex];
				if (text != null && text != "")
				{
					arg = ManagedSingleton<PlayerNetManager>.Instance.dicCardDeployNameInfo[CurrentDeployIndex];
				}
			}
			string msg = string.Format(MonoBehaviourSingleton<LocalizationManager>.Instance.GetStr("CARD_DEPLOY_SURE"), arg);
	...
	public void OnClearCurrentAllSetCard()
	{
		...
		MonoBehaviourSingleton<UIManager>.Instance.LoadResourceUI("UI_CommonMsg", delegate(CommonUI ui)
		{
			...
			ui.SetupYesNO(MonoBehaviourSingleton<LocalizationManager>.Instance.GetStr("COMMON_TIP"), msg, MonoBehaviourSingleton<LocalizationManager>.Instance.GetStr("COMMON_YES"), MonoBehaviourSingleton<LocalizationManager>.Instance.GetStr("COMMON_CANCEL"), delegate
			{
				...
				ManagedSingleton<PlayerNetManager>.Instance.EquipCardDeployReq(CuttentCharaterInfo.netInfo.CharacterID, source.ToList(), delegate
				{
					Setup(CuttentCharaterInfo, bIsDeploy, bIsSetDeploy);
				});
			});
