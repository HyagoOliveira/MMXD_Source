...
namespace UnityEngine.UI
{
	...
	public abstract class LoopScrollRect : UIBehaviour, IInitializePotentialDragHandler, IEventSystemHandler, IBeginDragHandler, IEndDragHandler, IDragHandler, IScrollHandler, ICanvasElement, ILayoutElement, ILayoutGroup, ILayoutController
	{
		...
		public float scrollConfirmTime = 0.1f;
		...
		public float ThresholdPlus = 1f;

		private int m_ContentConstraintCount;

		private int snap = -1;

		private int snapOffset = -1;
        [Obsolete]
        private CallbackObj beginDragCB;
        [Obsolete]
        private CallbackObj endDragCB;

		private Transform snapObj;
		...
		{
			base.Awake();
			if (prefabSource != null && prefabSource.pbo != null)
			{
				ScrollIndexCallback scrollIndexCallback = Object.Instantiate(prefabSource.pbo, base.transform);
				scrollIndexCallback.name = prefabSource.pbo.name;
				scrollIndexCallback.transform.localPosition = new Vector3(5000f, 5000f);
				prefabSource.pbo = scrollIndexCallback;
			}
		...
		protected override void OnDestroy()
		{
			...
			if (prefabSource != null && prefabSource.pbo != null)
			{
				MonoBehaviourSingleton<PoolManager>.Instance.ClearPoolItem(prefabSource.pbo.itemName);
			}
		}
		...
		public void OrangeInit<T>(T p_obj, int p_visualCount, int p_totalCount, int offset = 0) where T : ScrollIndexCallback
		{
			if (prefabSource.Inited)
			{
				prefabSource.Inited = prefabSource.pbo == p_obj;
			}
			prefabSource.pbo = p_obj;
			prefabSource.count = p_visualCount + 2;
			totalCount = p_totalCount;
			prefabSource.Init();
			if (offset > p_totalCount - 1)
			{
				offset = 0;
			}
			RefillCells(offset);
        ...
        [Obsolete]
        public void ApplySnap(int p_snap, int p_snapOffset = 0, int p_defaultIdx = 0, CallbackObj p_beginDargCB = null, CallbackObj p_endDragCB = null)
		{
			snap = p_snap;
			snapOffset = p_snapOffset;
			m_Inertia = false;
			movementType = MovementType.Elastic;
			beginDragCB = p_beginDargCB;
			endDragCB = p_endDragCB;
			LayoutRebuilder.ForceRebuildLayoutImmediate(content);
			while (p_defaultIdx > itemTypeEnd)
			{
				NewItemAtEnd();
			}
			SrollToCell(p_defaultIdx, 10000f, delegate
			{
				movementType = MovementType.Unrestricted;
				OnTween(content.GetChild(p_defaultIdx));
			});
		}

        [Obsolete]
        public void ApplySnapSpeed(int p_snap, int p_snapOffset = 0, int p_defaultIdx = 0, float p_speed = 10000f, CallbackObj p_beginDargCB = null, CallbackObj p_endDragCB = null)
		{
			snap = p_snap;
			snapOffset = p_snapOffset;
			m_Inertia = false;
			...
			SrollToCell(p_defaultIdx, p_speed, delegate
			{
				movementType = MovementType.Unrestricted;
				OnTween(content.GetChild(p_defaultIdx));
			});
		...
		public void SrollToCell(int index, float speed, Callback p_completeCB = null)
		{
			...
			{
				StopAllCoroutines();
				StartCoroutine(ScrollToCellCoroutine(index, speed, p_completeCB));
			}
		}

		private IEnumerator ScrollToCellCoroutine(int index, float speed, Callback p_completeCB = null)
		{
		...
		public void RefreshCellsNew()
		{
			if (Application.isPlaying && base.isActiveAndEnabled)
			{
				ScrollIndexCallback[] componentsInChildren = content.GetComponentsInChildren<ScrollIndexCallback>();
				for (int i = 0; i < componentsInChildren.Length; i++)
				{
					componentsInChildren[i].RefreshCell();
				}
			}
		}
		...
		public bool IsItemEndCulling()
		{
			int index = m_Content.childCount - 1;
			if (reverseDirection)
			{
				index = 0;
			}
			return m_Content.GetChild(index).GetComponent<CanvasRenderer>().cull;
		}
		...
		protected float NewItemAtStart()
		{
			...
			threshold = Mathf.Max(threshold, num * 3f * ThresholdPlus);
			if (!reverseDirection)
			{
				Vector2 vector = GetVector(num);
				content.anchoredPosition += vector;
		...
		public virtual void OnBeginDrag(PointerEventData eventData)
		{
			if (eventData.button == PointerEventData.InputButton.Left && IsActive())
			{
				...
				if (snap != -1)
				{
					beginDragCB.CheckTargetToInvoke(snapObj);
				}
			}
		}
		...
		private void OnSnap()
		{
			...
			float num4 = content.anchoredPosition[snap];
			for (int i = 0; i < childCount; i++)
			{
				float num3 = Mathf.Abs(content.GetChild(i).position[snap]);
				if (num3 < num2)
				{
					num = i;
					num2 = num3;
			...
			num = Mathf.Clamp(num + snapOffset, 0, itemTypeEnd);
			OnTween(content.GetChild(num));
		}

		public void OnTween(Transform target)
		{
			snapObj = target;
			float from = content.anchoredPosition[snap];
			float to = from - base.transform.InverseTransformPoint(target.position)[snap];
			LeanTween.value(base.gameObject, from, to, 0.15f).setOnUpdate(delegate(float f)
			{
				content.anchoredPosition = ((snap == 0) ? new Vector2(f, 0f) : new Vector2(0f, f));
			}).setOnComplete((Action)delegate
			{
				content.RebuildLayout();
				from = content.anchoredPosition[snap];
				to = from - base.transform.InverseTransformPoint(target.position)[snap];
				content.anchoredPosition = ((snap == 0) ? new Vector2(to, 0f) : new Vector2(0f, to));
				UpdateBounds(true);
				endDragCB.CheckTargetToInvoke(target);
			});
		}
		...
		{
			Vector2 localPoint;
			if (eventData.button != 0 || !IsActive() || !RectTransformUtility.ScreenPointToLocalPointInRectangle(viewRect, eventData.position, eventData.pressEventCamera, out localPoint))
			{
				return;
			}
			UpdateBounds();
