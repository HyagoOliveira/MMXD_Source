...
public class BS040_Controller : EnemyControllerBase, IManagedUpdateBehavior
{
	...
	private CollideBullet _venomCollideBulet;
	...
	protected override void Awake()
	{
		...
		_venomCollideBulet = OrangeBattleUtility.FindChildRecursive(ref target, "VenomCollider").gameObject.AddOrGetComponent<CollideBullet>();
	...
	private void UpdateRandomState()
	{
		...
		if (StageUpdate.bIsHost)
		{
			...
			case MainStatus.NextAction:
				mainStatus = (MainStatus)OrangeBattleUtility.Random(3, 8);
				if (mainStatus == MainStatus.Skill_02 && _lateAction == MainStatus.Skill_02)
				{
					mainStatus = ((!((float)(int)Hp > (float)(int)MaxHp * 0.5f)) ? MainStatus.Skill_04 : MainStatus.Skill_01);
				}
				break;
	...
	public override void LogicUpdate()
	{
		...
		case MainStatus.Skill_02:
			...
			case SubStatus.Phase2:
				if (AiTimer.GetMillisecond() > _diveTime)
				{
					...
					AcidLiquor(false);
					_venomCollideBulet.BackToPool();
					SetStatus(MainStatus.Skill_02, SubStatus.Phase3);
					Pause_skill02PPS();
				}
				break;
			case SubStatus.Phase3:
				...
				{
					SetStatus(MainStatus.Skill_02, SubStatus.Phase4);
					Play_skill02PPS();
				}
				break;
			case SubStatus.Phase4:
				if (AiTimer.GetMillisecond() > 1000)
				{
					...
					SetStatus(MainStatus.Skill_02, SubStatus.Phase5);
				}
				break;
			case SubStatus.Phase5:
				if (_currentFrame > 1f)
		...
		case MainStatus.Skill_03:
			...
			case SubStatus.Phase4:
				...
				if (_currentFrame > 1f)
				{
					SetStatus(MainStatus.Skill_03, SubStatus.Phase5);
				}
				break;
			case SubStatus.Phase5:
		...
		case MainStatus.Skill_04:
			...
			case SubStatus.Phase2:
				...
				else if (_currentFrame > 0.15f && _skill04BulletA.IsIdle() && _currentFrame < 0.25f)
				{
					_skill04BulletA.Shoot();
				}
				else if (_currentFrame > 0.55f && _skill04BulletB.IsIdle() && _currentFrame < 0.65f)
				{
		...
		if (_checkVenomTime && _skill02PPS.time > 0.9f)
		{
			...
			_venomCollideBulet.Active(targetMask);
		}
	}
	...
	public override void SetActive(bool isActive)
	{
		...
		if (isActive)
		{
			...
			_collideBullet.SetBulletAtk(null, selfBuffManager.sBuffStatus, EnemyData);
			_collideBullet.Active(targetMask);
			_skill02collideBullet.UpdateBulletData(EnemyWeapons[2].BulletData);
			_skill02collideBullet.SetBulletAtk(null, selfBuffManager.sBuffStatus, EnemyData);
			SKILL_TABLE pData = ManagedSingleton<OrangeDataManager>.Instance.SKILL_TABLE_DICT[EnemyWeapons[2].BulletData.n_LINK_SKILL];
			_venomCollideBulet.UpdateBulletData(pData);
			_venomCollideBulet.SetBulletAtk(null, selfBuffManager.sBuffStatus, EnemyData);
			_venomCollideBulet.nWeaponCheck = _skill02collideBullet.nWeaponCheck;
			_venomCollideBulet.nWeaponType = _skill02collideBullet.nWeaponType;
	...
	private void SetStatus(MainStatus mainStatus, SubStatus subStatus = SubStatus.Phase0)
	{
		...
		case MainStatus.Skill_01:
			...
			case SubStatus.Phase1:
			{
				...
				_skill01Bullet = (SeaHorseSkill01Bullet)BulletBase.TryShotBullet(EnemyWeapons[1].BulletData, _transform.position + localPosition, Vector3.right * base.direction, null, selfBuffManager.sBuffStatus, EnemyData, targetMask);
				break;
			}
			}
			break;
		case MainStatus.Skill_02:
			...
			case SubStatus.Phase2:
				...
				{
					_diveTime = OrangeBattleUtility.Random(1200, 2000);
				}
				break;
			case SubStatus.Phase3:
			{
				VInt3 targetPos2 = TargetPos;
				targetPos2.y = Controller.LogicPosition.y + 500;
				RaycastHit2D raycastHit2D2 = Physics2D.Raycast(targetPos2.vec3, Vector2.left, 5f, Controller.collisionMask);
				RaycastHit2D raycastHit2D3 = Physics2D.Raycast(targetPos2.vec3, Vector2.right, 5f, Controller.collisionMask);
				float num = (raycastHit2D2 ? (targetPos2.vec3.x - raycastHit2D2.distance + 1.5f) : (targetPos2.vec3.x - 2f));
				float num2 = (raycastHit2D3 ? (targetPos2.vec3.x + raycastHit2D3.distance - 1.5f) : (targetPos2.vec3.x + 2f));
				...
				zero.x = Mathf.RoundToInt(OrangeBattleUtility.Random(num, num2) * 1000f);
				zero.y = Controller.LogicPosition.y;
				Controller.LogicPosition = zero;
				UpdateDirection();
				break;
			}
			case SubStatus.Phase4:
		...
		case MainStatus.Skill_04:
			...
			case SubStatus.Phase0:
				...
				break;
			case SubStatus.Phase1:
				_skill04BulletA = (SeaHorseSkill04Bullet)BulletBase.TryShotBullet(EnemyWeapons[3].BulletData, _transform.position + Vector3.up, Vector3.right * base.direction, null, selfBuffManager.sBuffStatus, EnemyData, targetMask);
				_skill04BulletB = (SeaHorseSkill04Bullet)BulletBase.TryShotBullet(EnemyWeapons[3].BulletData, _transform.position + Vector3.up, Vector3.right * base.direction, null, selfBuffManager.sBuffStatus, EnemyData, targetMask);
				break;
			}
			break;
		case MainStatus.Die:
