...
public class EM127_Controller : EnemyControllerBase, IManagedUpdateBehavior
{
	...
	private VInt3 JumpForce = new VInt3(12000, 16000, 0);

	private float BoomRange = 2f;

	[SerializeField]
	private Transform BombPos;

	private CollideBullet BombCollider;
	...
	protected virtual void HashAnimation()
	{
		...
		_animationHash[1] = Animator.StringToHash("BS067@debut_0");
		_animationHash[2] = Animator.StringToHash("BS067@run2_loop");
		_animationHash[3] = Animator.StringToHash("BS067@run_loop");
		_animationHash[4] = Animator.StringToHash("BS067@skill_2_step1_start");
		_animationHash[5] = Animator.StringToHash("BS067@skill_2_step1_loop");
		_animationHash[6] = Animator.StringToHash("BS067@skill_2_step2_start");
		_animationHash[7] = Animator.StringToHash("BS067@skill_2_step2_loop");
		_animationHash[8] = Animator.StringToHash("BS067@skill_2_step2_end");
		_animationHash[9] = Animator.StringToHash("BS067@hurt_loop");
		_animationHash[10] = Animator.StringToHash("BS067@death");
	}
	...
	protected override void Start()
	{
		...
		MonoBehaviourSingleton<FxManager>.Instance.PreloadFx("fxuse_em127_explosion");
		MonoBehaviourSingleton<FxManager>.Instance.PreloadFx("fxstory_explode_000");
	}
	...
	private void SetStatus(MainStatus mainStatus, SubStatus subStatus = SubStatus.Phase0)
	{
		...
		case MainStatus.Atk:
			...
			case SubStatus.Phase0:
				...
				if ((bool)Target)
				{
					...
					UpdateDirection();
					if (Target._transform.position.y - _transform.position.y > 5f)
					{
						AtkEndPosX = Target._transform.position.x - 1.8f * (float)base.direction;
					}
					else
					{
						AtkEndPosX = Target._transform.position.x - 3.5f * (float)base.direction;
					}
				}
				PlaySE("BossSE03", "bs022_kobun05");
			...
			case SubStatus.Phase2:
				...
				if ((bool)Target)
				{
					...
					int num3 = -(JumpForce.y * 2) / num2;
					int num4 = num * 20 / num3;
					JumpForce.x = num4 * base.direction;
					_velocity = JumpForce;
					PlaySE("BossSE03", "bs022_kobun21");
				}
				break;
			case SubStatus.Phase3:
				BombPos.position = Weapon.transform.position;
				BombPos.SetParentNull();
				BombCollider.Active(targetMask);
				MonoBehaviourSingleton<FxManager>.Instance.Play<FxBase>("fxuse_em127_explosion", BombPos, Quaternion.identity, Vector3.one * 2.5f, Array.Empty<object>());
				ShowWeapon(false);
				_velocity = VInt3.zero;
				break;
		...
		case MainStatus.Die:
			...
			case SubStatus.Phase2:
				...
				break;
			case SubStatus.Phase3:
				PlaySE("BossSE03", "bs022_kobun25", 0.5f);
				IgnoreGravity = true;
				break;
			}
			break;
	...
	public override void LogicUpdate()
	{
		...
		case MainStatus.Atk:
		{
			...
			case SubStatus.Phase0:
				if ((bool)Target)
				{
					num = Vector3.Distance(_transform.position, Target._transform.position);
				}
				if (num < BoomRange)
				{
				...
				else if ((_transform.position.x - AtkEndPosX) * (float)base.direction > 0f)
				{
					_velocity.x = 0;
			...
			case SubStatus.Phase2:
				...
				else if (Controller.Collisions.below && _currentFrame > 0.5f)
				{
					SetStatus(MainStatus.Atk, SubStatus.Phase3);
				}
				break;
			case SubStatus.Phase3:
				if (_currentFrame > 1f)
				{
					BombCollider.BackToPool();
					SetStatus(MainStatus.Atk, SubStatus.Phase4);
				}
				break;
			case SubStatus.Phase4:
				if (_currentFrame > 3f && Controller.Collisions.below)
				{
		...
		case MainStatus.Die:
		{
			...
			if (shortNameHash != _animationHash[10] && shortNameHash != _animationHash[9])
			{
				break;
			}
			switch (_subStatus)
	...
	public override void SetActive(bool isActive)
	{
		...
		if (isActive)
		{
			...
			BombCollider.UpdateBulletData(EnemyWeapons[1].BulletData);
			BombCollider.SetBulletAtk(null, selfBuffManager.sBuffStatus, EnemyData);
	...
	private void LoadParts(Transform[] childs)
	{
		if (!Weapon)
		{
			Weapon = OrangeBattleUtility.FindChildRecursive(ref childs, "BombMesh_c", true).gameObject;
		}
		if (!BombPos)
		{
			BombPos = OrangeBattleUtility.FindChildRecursive(ref childs, "Bone_Bomb", true);
		}
		if (!BombCollider)
		{
			BombCollider = OrangeBattleUtility.FindChildRecursive(ref childs, "BombCollider", true).gameObject.AddOrGetComponent<CollideBullet>();
		}
		if (!ModelTransform)
		{
