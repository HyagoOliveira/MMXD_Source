...
public class EM126_Controller : EnemyControllerBase, IManagedUpdateBehavior
{
	...
	private bool hasTurn1;

	private bool hasTurn2;
	...
	protected virtual void HashAnimation()
	{
		...
		_animationHash[1] = Animator.StringToHash("BS067@debut_2");
		_animationHash[2] = Animator.StringToHash("BS067@run2_loop");
		_animationHash[3] = Animator.StringToHash("BS067@run_loop");
		_animationHash[4] = Animator.StringToHash("BS067@skill_1_start");
		_animationHash[5] = Animator.StringToHash("BS067@skill_1_loop");
		_animationHash[6] = Animator.StringToHash("BS067@skill_1_end");
		...
		case MainStatus.Atk:
			...
			case SubStatus.Phase0:
				...
				if (!HasRun)
				{
					...
					AtkEndPosX = vector.x - Vector3.right.x * (float)base.direction * 0.3f;
					PlaySE("BossSE03", "bs022_kobun17");
					HasRun = true;
			...
			case SubStatus.Phase2:
				...
				PlaySE("BossSE03", "bs022_kobun04");
				BulletBase.TryShotBullet(EnemyWeapons[1].BulletData, Vector3.right * base.direction * 0.5f + ShootPos.position, new Vector3(base.direction, 0f, 0f), null, selfBuffManager.sBuffStatus, EnemyData, targetMask).needPlayEndSE = true;
				break;
	...
	public override void LogicUpdate()
	{
		...
		case MainStatus.Debut:
			...
			case SubStatus.Phase0:
				...
				break;
			case SubStatus.Phase1:
				if (_currentFrame > 0.3f && !hasTurn1)
				{
					hasTurn1 = true;
					UpdateDirection(1);
				}
				else if (_currentFrame > 0.8f && !hasTurn2)
				{
					hasTurn2 = true;
					UpdateDirection(-1);
				}
		...
		case MainStatus.Atk:
			...
			case SubStatus.Phase0:
				...
				break;
			case SubStatus.Phase1:
				if (!WeaponOpen && _currentFrame > 0.36f)
				{
					ShowWeapon(true);
				}
			...
			case SubStatus.Phase3:
				...
				break;
			case SubStatus.Phase4:
				if (WeaponOpen && _currentFrame > 0.7f)
				{
					ShowWeapon(false);
				}
	...
	private void LoadParts(Transform[] childs)
	{
		if (!Weapon)
		{
			Weapon = OrangeBattleUtility.FindChildRecursive(ref childs, "BS067_CannonMesh", true).gameObject;
		}
		if (!ModelTransform)
		{
		...
		if (!ShootPos)
		{
			ShootPos = OrangeBattleUtility.FindChildRecursive(ref childs, "ShootPoint_Cannon", true);
		}
	}
