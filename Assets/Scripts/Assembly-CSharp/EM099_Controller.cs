...
public class EM099_Controller : EnemyControllerBase, IManagedUpdateBehavior
{
	...
	private CollideBullet ExplosionCollider;

	private bool _isExplosion;

	private Transform InfoBar;

	private OrangeTimer ExpTimer;
	...
	protected override void Awake()
	{
		...
		ExplosionCollider = OrangeBattleUtility.FindChildRecursive(ref target, "Collider").gameObject.AddOrGetComponent<CollideBullet>();
		InfoBar = OrangeBattleUtility.FindChildRecursive(ref target, "InfoBar");
		...
		ExpTimer = OrangeTimerManager.GetTimer();
	...
	public override void LogicUpdate()
	{
		...
		case MainStatus.Idle:
			...
			if ((bool)Target)
			{
				...
				NextPosX = DistanceX * (float)base.direction + Target.transform.position.x;
				if (hasRun)
				{
		...
		case MainStatus.Fall:
			...
			break;
		case MainStatus.Walk:
			if ((_transform.position.x - NextPosX) * (float)base.direction >= 0f)
			{
				SetStatus(MainStatus.Idle);
			}
		...
		if (!_isExplosion && ExpTimer.GetMillisecond() > 50000)
		{
			SetStatus(MainStatus.Idle);
			StartExplosion();
		}
		else if (_parentController == null || (int)_parentController.Hp < 0)
		{
			_isExplosion = false;
			ExpTimer.TimerStop();
			Explosion();
		}
		else if (_isExplosion && ExpTimer.GetMillisecond() > 500)
		{
	...
	public override void SetActive(bool isActive)
	{
		...
		if (isActive)
		{
			ModelTransform.localScale = new Vector3(5.5f, 5.5f, 5.5f);
			_animator = ModelTransform.GetComponent<Animator>();
			base.AimPoint = new Vector3(0f, 0.5f, 0f);
			SetParameter(1, OrangeBattleUtility.Random(0, 2) * 2 - 1, new VInt3(600, 7000, 0));
			IgnoreGravity = false;
			ExplosionCollider.UpdateBulletData(EnemyWeapons[0].BulletData);
			ExplosionCollider.SetBulletAtk(null, selfBuffManager.sBuffStatus, EnemyData);
	...
	{
		if (((1 << col.gameObject.layer) & (int)targetMask) != 0 && !col.isTrigger && !_isExplosion && Activate)
		{
			StartExplosion();
		}
	}
	...
	protected void StartExplosion()
	{
		...
		MonoBehaviourSingleton<FxManager>.Instance.Play<FxBase>("fxuse_em_explosion", ExplosionCollider.transform, Quaternion.identity, new Vector3(1f, 1f, 1f), Array.Empty<object>());
		ExplosionCollider.Active(targetMask);
		ModelTransform.localScale = new Vector3(0f, 0f, 0f);
		_isExplosion = true;
		ExpTimer.TimerReset();
		ExpTimer.TimerStart();
		SetStatus(MainStatus.Explosion);
	}
