...
public class BS092_Controller : EnemyControllerBase, IManagedUpdateBehavior
{
	...
	public override void LogicUpdate()
	{
		...
		if ((!Activate && _mainStatus != MainStatus.Debut) || !CanMove)
		{
			return;
		}
		base.LogicUpdate();
		...
		case MainStatus.PullClaw:
			...
			{
			case SubStatus.Phase0:
				if (targetOC != null && (bool)OrangeBattleUtility.RaycastIgnoreSelf(LeftCatchPoint.position, Vector2.right * base.direction, 1.8f, Controller.collisionMask, _transform) && IsCatch)
				{
					releaseOC();
				}
	...
	protected virtual void UpdateRandomState()
	{
		...
		if (mainStatus == MainStatus.PushAttack)
		{
	...
	private void releaseOC()
	{
		if (IsCatch && (bool)targetOC)
		{
			targetOC._transform.position = new Vector3(targetOC.transform.position.x, targetOC.transform.position.y, 0f);
			targetOC.Controller.LogicPosition = new VInt3(targetOC._transform.position);
