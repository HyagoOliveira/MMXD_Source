...
public class EM034_Controller : EnemyControllerBase, IManagedUpdateBehavior
{
	...
	protected override void Awake()
	{
		...
		_animatorHash[0] = Animator.StringToHash("EM034@idle_loop");
		_animatorHash[1] = Animator.StringToHash("EM034@hurt_loop");
		_animatorHash[3] = Animator.StringToHash("EM034@shot_left");
		_animatorHash[4] = Animator.StringToHash("EM034@shot_right");
		_animatorHash[5] = Animator.StringToHash("EM034@idle_loop");
	...
	public override void LogicUpdate()
	{
		...
		case MainStatus.IDLE:
			if (AiState == AI_STATE.mob_002 || AiTimer.GetMillisecond() < EnemyData.n_AI_TIMER)
			{
				break;
			}
			Target = _enemyAutoAimSystem.GetClosetPlayer();
			if (Target == null || !IsWeaponAvailable(1))
			{
				break;
			}
			...
			{
				SetStatus(MainStatus.SHOOT_LEFT);
			}
			break;
		case MainStatus.SHOOT_LEFT:
		{
			int num = 1;
			if (EnemyWeapons[num].MagazineRemain > 0f && (!EnemyWeapons[num].LastUseTimer.IsStarted() || EnemyWeapons[num].LastUseTimer.GetMillisecond() > EnemyWeapons[num].BulletData.n_FIRE_SPEED))
			{
				...
				GroundMissileBullet groundMissileBullet = (GroundMissileBullet)BulletBase.TryShotBullet(pTransform: (num2 != 0 && num2 == 1) ? _rightGunTransform : _leftGunTransform, tSkillTable: EnemyWeapons[num].BulletData, pDirection: Vector3.up, weaponStatus: null, tBuffStatus: selfBuffManager.sBuffStatus, refMOB_TABLE: EnemyData, pTargetMask: targetMask);
		...
		if (_mainStatus == MainStatus.SHOOT_LEFT)
		{
			_animator.Play(((int)EnemyWeapons[1].MagazineRemain % 2 == 0) ? _animatorHash[3] : _animatorHash[4]);
		}
		else
		{
